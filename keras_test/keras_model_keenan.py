...
def train_top_model():
    ...
    train_data = np.load(open('bottleneck_features_train_3.npy'))
    validation_data = np.load(open('bottleneck_features_validation_3.npy'))
    ...
    with tf.device('/gpu:2'):
        ...
        model.compile('adam', loss='categorical_crossentropy', metrics=['accuracy']) #was sparce_cat...
        ...
        # K-Fold Validation
        # * Taken from https://github.com/fchollet/keras/issues/1711
        # n_folds = 10
        # skf = StratifiedKFold(train_labels,n_folds=n_folds,shuffle=True)
        ...
        #     model.add(Activation('softmax'))
        #     model.compile('adam', loss='categorical_crossentropy', metrics=['accuracy']) #was sparce_cat...
        #     model.fit(train_data[train], train_labels_cat[train],
        #               nb_epoch=nb_epoch, batch_size=50, callbacks=callbacks_list,
        #               validation_data=(train_data[test], train_labels_cat[test]))

        # np.save('folds', folds)
        folds = np.load('folds.npy')
        [train,test] = folds[3]     # The best from doing cross-validation.
        model = None
        model = Sequential()
        model.add(Flatten(input_shape=(512, 7, 7)))  # this converts our 3D feature maps to 1D feature vectors
        ...
        model.compile('adam', loss='categorical_crossentropy', metrics=['accuracy']) #was sparce_cat...
        model.fit(train_data[train], train_labels_cat[train],
                  nb_epoch=nb_epoch, batch_size=500, callbacks=callbacks_list,
                  validation_data=(train_data[test], train_labels_cat[test]))
